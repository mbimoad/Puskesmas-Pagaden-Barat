...
function cmb_dinamis($name,$table,$field,$pk,$selected=null){
    $ci = get_instance();
    $cmb = "<select name='$name' class='form-control'>"."<option value=''>Pilih</option>";
...
$ci = get_instance();
    // mencari kode barang dengan nilai paling besar
    $query = "SELECT max(no_rawat) as maxKode FROM tbl_pendaftaranbpjs";
    $data = $ci->db->query($query)->row_array();
    $kode = $data['maxKode'];
    $noUrut = (int) substr($kode, 0, 6);
    $noUrut++;
...
function noRegistrasiotomatis2(){
$ci = get_instance();
    // mencari kode barang dengan nilai paling besar
    $query = "SELECT max(no_rawat) as maxKode FROM tbl_pendaftaranumum";
    $data = $ci->db->query($query)->row_array();
    $kode = $data['maxKode'];
    $noUrut = (int) substr($kode, 0, 6);
    $noUrut++;

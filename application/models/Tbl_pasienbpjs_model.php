...
class Tbl_pasienbpjs_model extends CI_Model
{

    public $table = 'tbl_pasienbpjs';
    ...
    // Get Total 
    function getTotalBPJS($q, $awal, $akhir) {
         // Ambil Tabel
        $this->db->distinct();
        $this->db->from($this->table); 
        $cek_tanggal = ($awal <> '') && ($akhir <> '' );

        // Jika Yang diinput Data bukan tanggal. Cari berdasarkan no bpjs
        if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
            $this->db->where('status_pasien', 'BPJS')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
            $this->db->where('status_pasien', 'BPJS')->where('tanggal_pendaftaran >=', $awal)->where('tanggal_pendaftaran <=', $akhir);
        // Jika tidak ada yang diinput
        } else {
            $this->db->where('status_pasien', 'BPJS');
        }
    ...
    function getPasienLama($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->where('ketpasien', 'LAMA')->like('no_bpjs', $q);
    ...
    function getPasienBaru($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->where('ketpasien', 'BARU')->like('no_bpjs', $q);
    ...
    function getPasienLaki($q = NULL, $awal = NULL, $akhir = NULL) {
            ...
            if(($q <> '') && ($cek_tanggal <> 1) ) {
                // Query And didouble -> pada where
                $this->db->like('jenis_kelamin', 'L')->like('no_bpjs', $q);
            // Jika yang diinput tanggal
            } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPasienPerempuan($q = NULL, $awal = NULL, $akhir = NULL) {
          ...
          if(($q <> '') && ($cek_tanggal <> 1) ) {
          // Query And didouble -> pada where
          $this->db->like('jenis_kelamin', 'P')->like('no_bpjs', $q);
          // Jika yang diinput tanggal
          } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getBalingbing($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->like('alamat', 'BALINGBING')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getCidahu($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->like('alamat', 'CIDAHU')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getMunjul($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
        $this->db->like('alamat', 'MUNJUL')->like('no_bpjs', $q);        
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getMargahayu($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
        $this->db->like('alamat', 'MARGAHAYU')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getBendungan($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
        $this->db->like('alamat', 'BENDUNGAN')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getCidadap($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->like('alamat', 'CIDADAP')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getMekarwangi($q = NULL, $awal = NULL, $akhir = NULL) {
       ...
       if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('alamat', 'MEKARWANGI')->like('no_bpjs', $q);
       // Jika yang diinput tanggal
       } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPangsor($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
        $this->db->like('alamat', 'PANGSOR')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getSumurgintung($q = NULL, $awal = NULL, $akhir = NULL) {
       ...
       if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('alamat', 'SUMURGINTUNG')->like('no_bpjs', $q);
       // Jika yang diinput tanggal
       } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getLuar($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('alamat', 'LUAR')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPoligigi($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('id_poli', 'POLI GIGI')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPoliumum($q = NULL, $awal = NULL, $akhir = NULL) {
        ...
        if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('id_poli', 'POLI UMUM')->like('no_bpjs', $q);
        // Jika yang diinput tanggal
        } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPolimtbs($q = NULL, $awal = NULL, $akhir = NULL) {
         ...
         if(($q <> '') && ($cek_tanggal <> 1) ) {
            // Query And didouble -> pada where
            $this->db->like('id_poli', 'POLI MTBS')->like('no_bpjs', $q);
            // Jika yang diinput tanggal
         } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    function getPolikia($q = NULL, $awal = NULL, $akhir = NULL) {
         ...
         if(($q <> '') && ($cek_tanggal <> 1) ) {
        // Query And didouble -> pada where
        $this->db->like('id_poli', 'POLI KIA/KB')->like('no_bpjs', $q);   
         // Jika yang diinput tanggal
         } else if(($q == '') && ($cek_tanggal == 1)) {
    ...
    // Cari data yang akan ditampilkan di index berdasarkan no bpjs

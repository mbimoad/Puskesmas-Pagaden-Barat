...
class Pasienbpjs extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Tbl_pasienbpjs_model');
        $this->load->library('form_validation');
    }
    
    public function hapudata() {
        $this->load->model('Tbl_pasienbpjs_model');
        $this->Tbl_pasienbpjs_model->deletealldata();
        redirect(site_url('pasienbpjs'));
    }
    ...
    public function index()
    {
        ...
        if (($q <> '') && ($cek_tanggal <> 1)) {
            $config['base_url'] = base_url() . 'index.php/pasienbpjs/index?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'index.php/pasienbpjs/index?q=' . urlencode($q);
            $config['per_page'] = 1000;
            $config['page_query_string'] = FALSE;
            $config['total_rows'] = $this->Tbl_pasienbpjs_model->total_rows($q);
            $config['pasienLama'] = $this->Tbl_pasienbpjs_model->getPasienLama($q);
            $pasien = $this->Tbl_pasienbpjs_model->get_limit_data($config['per_page'], $start, $q);
            $config['full_tag_open'] = '<ul class="pagination pagination-sm no-margin pull-right">';
            $config['full_tag_close'] = '</ul>';

            // Get Total 
            
            $config['total_databpjs']   = $this->Tbl_pasienbpjs_model->getTotalBPJS($q, $awal, $akhir);
            $config['total_pasienlama'] = $this->Tbl_pasienbpjs_model->getPasienLama($q, $awal, $akhir);
            $config['total_pasienbaru'] = $this->Tbl_pasienbpjs_model->getPasienBaru($q, $awal, $akhir);;
            $config['total_pasienlaki'] = $this->Tbl_pasienbpjs_model->getPasienLaki($q, $awal, $akhir);
            $config['total_pasienperempuan'] = $this->Tbl_pasienbpjs_model->getPasienPerempuan($q, $awal, $akhir);;
            $config['total_balingbing'] = $this->Tbl_pasienbpjs_model->getBalingbing($q, $awal, $akhir);
            $config['total_bendungan'] = $this->Tbl_pasienbpjs_model->getBendungan($q, $awal, $akhir);
            $config['total_cidahu'] = $this->Tbl_pasienbpjs_model->getCidahu($q, $awal, $akhir);
            $config['total_cidadap'] = $this->Tbl_pasienbpjs_model->getCidadap($q, $awal, $akhir);
            $config['total_margahayu'] = $this->Tbl_pasienbpjs_model->getMargahayu($q, $awal, $akhir);
            $config['total_mekarwangi'] = $this->Tbl_pasienbpjs_model->getMekarwangi($q, $awal, $akhir);
            $config['total_munjul'] = $this->Tbl_pasienbpjs_model->getMunjul($q, $awal, $akhir);
            $config['total_pangsor'] = $this->Tbl_pasienbpjs_model->getPangsor($q, $awal, $akhir);
            $config['total_sumurgintung'] = $this->Tbl_pasienbpjs_model->getSumurgintung($q, $awal, $akhir);
            $config['total_luar'] = $this->Tbl_pasienbpjs_model->getLuar($q, $awal, $akhir);
            $config['total_umum'] = $this->Tbl_pasienbpjs_model->getPoliumum($q, $awal, $akhir);
            $config['total_gigi'] = $this->Tbl_pasienbpjs_model->getPoligigi($q, $awal, $akhir);
            $config['total_mtbs'] = $this->Tbl_pasienbpjs_model->getPolimtbs($q, $awal, $akhir);
            $config['total_kia'] = $this->Tbl_pasienbpjs_model->getPolikia($q, $awal, $akhir);
            ...
            $data = array (
                ...
                'start' => $start,  
                 ...
                 // Total
                 'total_databpjs' => $config['total_databpjs'],
        ...
        // Jika User Menginput tanggal between (TRUE)
            ...
            $config['base_url'] = base_url() . 'index.php/pasienbpjs/awal?awal=' . urlencode($awal) . "&akhir=" . urlencode($akhir);
            $config['first_url'] = base_url() . 'index.php/pasienbpjs/awal?awal=' . urlencode($awal) . "&akhir=" . urlencode($akhir);
            $config['per_page'] = 1000;
            $config['page_query_string'] = FALSE;
            $config['total_rows'] = $this->Tbl_pasienbpjs_model->total_rows($awal);
            $config['total_rows'] = $this->Tbl_pasienbpjs_model->total_rows($akhir);
            $config['pasienLama'] = $this->Tbl_pasienbpjs_model->getPasienLama($awal);
            $start = intval($this->uri->segment(3)); // NILAI PAGINATION
            $pasien = $this->Tbl_pasienbpjs_model->get_between_data($config['per_page'], $start, $awal, $akhir);
            $config['full_tag_open'] = '<ul class="pagination pagination-sm no-margin pull-right">';
            $config['full_tag_close'] = '</ul>';
        ...
        // Jika User tidak menginputkan apapun
            ...
            $config['base_url'] = base_url() . 'index.php/pasienbpjs/index/';
            $config['first_url'] = base_url() . 'index.php/pasienbpjs/index/';
            $config['per_page'] = 10;
            $config['page_query_string'] = FALSE;
            $pasien = $this->Tbl_pasienbpjs_model->get_limit_data($config['per_page'], $start, $q);
            $config['total_rows'] = $this->Tbl_pasienbpjs_model->total_rows($q);
        ...
        $this->template->load('template','pasienbpjs/tbl_pasien_list', $data);
    }
    ...
    function ktpauto() {
       autocomplate_json('tbl_pasienbpjs', 'no_ktp');
    }
    ...
    public function excel() {
        ...
        $sheet->setCellValue('B1', 'Data Pasien BPJS');
		...
		$sheet->setCellValue('G3', 'NoBPJS');
		$sheet->setCellValue('H3', 'Nama Pasien');
		$sheet->setCellValue('I3', 'Bin/Binti');
		$sheet->setCellValue('J3', 'JK');
		$sheet->setCellValue('K3', 'Tgl Lahir');
		$sheet->setCellValue('L3', 'DESA');
		$sheet->setCellValue('M3', 'RT/RW');
		$sheet->setCellValue('N3', 'ID Poli');
        ...
        if(($uri3 == NULL) && ($uri4 == NULL)) {
            $pasien_bpjs = $this->Tbl_pasienbpjs_model->getAll();

            // Total
            $total_databpjs = $this->Tbl_pasienbpjs_model->getTotalBPJS(NULL, NULL, NULL);
            $total_pasienlama = $this->Tbl_pasienbpjs_model->getPasienLama(NULL, NULL, NULL);;
            $total_pasienbaru = $this->Tbl_pasienbpjs_model->getPasienBaru(NULL, NULL, NULL);;
            $total_pasienlaki = $this->Tbl_pasienbpjs_model->getPasienLaki(NULL, NULL, NULL);
            $total_pasienperempuan = $this->Tbl_pasienbpjs_model->getPasienPerempuan(NULL, NULL, NULL);;
            $total_balingbing = $this->Tbl_pasienbpjs_model->getBalingbing(NULL, NULL, NULL);
            $total_bendungan = $this->Tbl_pasienbpjs_model->getBendungan(NULL, NULL, NULL);
            $total_cidahu = $this->Tbl_pasienbpjs_model->getCidahu(NULL, NULL, NULL);
            $total_cidadap = $this->Tbl_pasienbpjs_model->getCidadap(NULL, NULL, NULL);
            $total_margahayu = $this->Tbl_pasienbpjs_model->getMargahayu(NULL, NULL, NULL);
            $total_mekarwangi = $this->Tbl_pasienbpjs_model->getMekarwangi(NULL, NULL, NULL);
            $total_munjul = $this->Tbl_pasienbpjs_model->getMunjul(NULL, NULL, NULL);
            $total_pangsor = $this->Tbl_pasienbpjs_model->getPangsor(NULL, NULL, NULL);
            $total_sumurgintung= $this->Tbl_pasienbpjs_model->getSumurgintung(NULL, NULL, NULL);
            $total_luar = $this->Tbl_pasienbpjs_model->getLuar(NULL, NULL, NULL);
            $total_umum = $this->Tbl_pasienbpjs_model->getPoliumum(NULL, NULL, NULL);
            $total_gigi = $this->Tbl_pasienbpjs_model->getPoligigi(NULL, NULL, NULL);
            $total_mtbs = $this->Tbl_pasienbpjs_model->getPolimtbs(NULL, NULL, NULL);
            $total_kia = $this->Tbl_pasienbpjs_model->getPolikia(NULL, NULL, NULL);
            
        }
        // Jika hanya mencari data
        else if($uri4 == NULL) { 
            $pasien_bpjs = $this->Tbl_pasienbpjs_model->getDataByFilter($uri3); 
            // Ambil Total
            $total_databpjs = $this->Tbl_pasienbpjs_model->getTotalBPJS($uri3, NULL, NULL);
            $total_pasienlama = $this->Tbl_pasienbpjs_model->getPasienLama($uri3, NULL, NULL);;
            $total_pasienbaru = $this->Tbl_pasienbpjs_model->getPasienBaru($uri3, NULL, NULL);;
            $total_pasienlaki = $this->Tbl_pasienbpjs_model->getPasienLaki($uri3, NULL, NULL);
            $total_pasienperempuan = $this->Tbl_pasienbpjs_model->getPasienPerempuan($uri3, NULL, NULL);;
            $total_balingbing = $this->Tbl_pasienbpjs_model->getBalingbing($uri3, NULL, NULL);
            $total_bendungan = $this->Tbl_pasienbpjs_model->getBendungan($uri3, NULL, NULL);
            $total_cidahu = $this->Tbl_pasienbpjs_model->getCidahu($uri3, NULL, NULL);
            $total_cidadap = $this->Tbl_pasienbpjs_model->getCidadap($uri3, NULL, NULL);
            $total_margahayu = $this->Tbl_pasienbpjs_model->getMargahayu($uri3, NULL, NULL);
            $total_mekarwangi = $this->Tbl_pasienbpjs_model->getMekarwangi($uri3, NULL, NULL);
            $total_munjul = $this->Tbl_pasienbpjs_model->getMunjul($uri3, NULL, NULL);
            $total_pangsor = $this->Tbl_pasienbpjs_model->getPangsor($uri3, NULL, NULL);
            $total_sumurgintung= $this->Tbl_pasienbpjs_model->getSumurgintung($uri3, NULL, NULL);
            $total_luar = $this->Tbl_pasienbpjs_model->getLuar($uri3, NULL, NULL);
            $total_umum = $this->Tbl_pasienbpjs_model->getPoliumum($uri3, NULL, NULL);
            $total_gigi = $this->Tbl_pasienbpjs_model->getPoligigi($uri3, NULL, NULL);
            $total_mtbs = $this->Tbl_pasienbpjs_model->getPolimtbs($uri3, NULL, NULL);
            $total_kia = $this->Tbl_pasienbpjs_model->getPolikia($uri3, NULL, NULL);
        ...
        // Jika Filter data dengan tanggal
            ...
            $pasien_bpjs = $this->Tbl_pasienbpjs_model->getDateByFilter($uri3, $uri4);
            // Total
            $total_databpjs = $this->Tbl_pasienbpjs_model->getTotalBPJS(NULL, $uri3, $uri4);
            $total_pasienlama = $this->Tbl_pasienbpjs_model->getPasienLama(NULL, $uri3, $uri4);;
            $total_pasienbaru = $this->Tbl_pasienbpjs_model->getPasienBaru(NULL, $uri3, $uri4);;
            $total_pasienlaki = $this->Tbl_pasienbpjs_model->getPasienLaki(NULL, $uri3, $uri4);
            $total_pasienperempuan = $this->Tbl_pasienbpjs_model->getPasienPerempuan(NULL, $uri3, $uri4);;
            $total_balingbing = $this->Tbl_pasienbpjs_model->getBalingbing(NULL, $uri3, $uri4);
            $total_bendungan = $this->Tbl_pasienbpjs_model->getBendungan(NULL, $uri3, $uri4);
            $total_cidahu = $this->Tbl_pasienbpjs_model->getCidahu(NULL, $uri3, $uri4);
            $total_cidadap = $this->Tbl_pasienbpjs_model->getCidadap(NULL, $uri3, $uri4);
            $total_margahayu = $this->Tbl_pasienbpjs_model->getMargahayu(NULL, $uri3, $uri4);
            $total_mekarwangi = $this->Tbl_pasienbpjs_model->getMekarwangi(NULL, $uri3, $uri4);
            $total_munjul = $this->Tbl_pasienbpjs_model->getMunjul(NULL, $uri3, $uri4);
            $total_pangsor = $this->Tbl_pasienbpjs_model->getPangsor(NULL, $uri3, $uri4);
            $total_sumurgintung= $this->Tbl_pasienbpjs_model->getSumurgintung(NULL, $uri3, $uri4);
            $total_luar = $this->Tbl_pasienbpjs_model->getLuar(NULL, $uri3, $uri4);
            $total_umum = $this->Tbl_pasienbpjs_model->getPoliumum(NULL, $uri3, $uri4);
            $total_gigi = $this->Tbl_pasienbpjs_model->getPoligigi(NULL, $uri3, $uri4);
            $total_mtbs = $this->Tbl_pasienbpjs_model->getPolimtbs(NULL, $uri3, $uri4);
            $total_kia = $this->Tbl_pasienbpjs_model->getPolikia(NULL, $uri3, $uri4);   
        }
	
		$no    = 1;
        $kolom = 4;
		foreach($pasien_bpjs as $row) {
			...
			$sheet->setCellValue('F'.$kolom, $row->no_ktp)
                                                ...
                                                ->getNumberFormat()
                                                ->setFormatCode('0');
            $sheet->setCellValue('G'.$kolom, $row->no_bpjs)
                                                ->getStyle('G' . $kolom)
                                                ->getNumberFormat()
                                                ->setFormatCode('0');
                                                

			$sheet->setCellValue('H'.$kolom, $row->nama_pasien);
			$sheet->setCellValue('I'.$kolom, $row->bin_binti);
			$sheet->setCellValue('J'.$kolom, $row->jenis_kelamin);
			$sheet->setCellValue('K'.$kolom, $row->tanggal_lahir);
			$sheet->setCellValue('L'.$kolom, $row->alamat);
			$sheet->setCellValue('M'.$kolom, $row->rt_rw_lengkap);
			$sheet->setCellValue('N'.$kolom, $row->id_poli);
			$kolom++;            
        }
        ...
        $sheet->setCellValue('A' . $tmp, 'BPJS');         
        $sheet->setCellValue('B' . $tmp, $total_databpjs );  
        ...
        $sheet->getStyle('A3:N' . ($kolom - 1))->applyFromArray($styleArray);
		$writer = new Xlsx($spreadsheet);
		$filename = 'laporan-pasien_bpjs';
    ...
    function cetak(){
        ...
        $pdf->Cell(70, 7, '', 0, 0, 'C');
        $pdf->Cell(220, 7, 'Laporan Data Pasien BPJS', 0, 1, 'C');
        ...
        $pdf->Cell(35, 7, 'No. BPJS  ', 1, 0, 'C');
        $pdf->Cell(67, 7, 'Nama Pasien  ', 1, 0, 'C');
        ...
        foreach ($pasien_bpjs as $r) {
            ...
            $pdf->Cell(35, 7, $r->no_bpjs, 1, 0, 'C');
            $pdf->Cell(67, 7, $r->nama_pasien, 1, 0, 'C');
        ...
        // Baris 1
        $pdf->Cell(130, 10, '', 0, 1);
        $pdf->Cell(40, 7, 'Total BPJS', 2, 0, 'C');
        $pdf->Cell(40, 7, $total_databpjs, 3, 0, 'C');
    ...
    function cetakkartu($id){
        ...
        $sql_pasien = $this->Tbl_pasienbpjs_model->get_by_id($id);
        $this->load->library('pdf');
        $pdf = new FPDF();
        $pdf->AddPage('P','A4');
    ...
    {
        $row = $this->Tbl_pasienbpjs_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'id_poli' => $row->id_poli,
            ...
            $this->template->load('template','pasienbpjs/tbl_pasien_read', $data);
        } else {
    ...
    function noRekmedBPJSOtomatis(){
    $ci = get_instance();
    // mencari kode barang dengan nilai paling besar
    $query = "SELECT max(no_rekamedis) as maxKode FROM tbl_pasienbpjs";
    $data = $ci->db->query($query)->row_array();
    $kode = $data['maxKode']; // Ambil Jumlah Terbaru Data 
    $noUrut = (int) substr($kode, 1, 5);
    $noUrut++;
    $kodeBaru = sprintf("B%05s", $noUrut);
    return $kodeBaru;
}
    ...
    $noRekammedisbaru = $this->noRekmedBPJSOtomatis();
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('pasienbpjs/create_action'),
        ...
        $this->template->load('template','pasienbpjs/tbl_pasien_form', $data);
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Tbl_pasienbpjs_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('pasienbpjs/update_action'),
    ...
    public function update_action() 
    {
        ...
        'id_poli' => $this->input->post('id_poli',TRUE),
            ...
            $this->Tbl_pasienbpjs_model->update($this->input->post('no_rekamedis', TRUE), $data);
            $this->session->set_flashdata('message', '<div class="alert alert-info">Data Berhasil Diupdate
            </div>');              redirect(site_url('pasienbpjs'));
        }
    }
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tbl_pasienbpjs_model->delete($id);

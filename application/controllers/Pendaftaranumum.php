...
class Pendaftaranumum extends CI_Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Tbl_pendaftaranumum_model');
   ...
   public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'index.php/pendaftaranumum/index?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'index.php/pendaftaranumum/index?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/pendaftaranumum/index';
            $config['first_url'] = base_url() . 'index.php/pendaftaranumum/index';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tbl_pendaftaranumum_model->total_rows($q);
        $pendaftaran = $this->Tbl_pendaftaranumum_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $this->template->load('template','pendaftaranumum/tbl_pendaftaran_list', $data);
    }

    function detail($no_rekmed){

      $no_rawat = substr($this->uri->uri_string(3), 19);
      $sql_no_rekmed = "SELECT no_rekamedis from tbl_pasienumum";
      $sql_daftar = "SELECT pd.no_rekamedis, pd.no_rawat,ps.nama_pasien FROM tbl_pendaftaran as pd, tbl_pasienumum as ps WHERE pd.no_rekamedis = ps.no_rekamedis and pd.no_rawat = '$no_rawat'";
      ...
      $sql_data_tindakan = "SELECT rt.no_rawat,rt.no_rekamedis,rt.hasil_periksa,rt.nama_obat,rt.tanggal,p.nama_poli, pkt.nama_penyakit, t.nama_tindakan FROM tbl_riwayat_tindakan as rt, tbl_tindakan as t, tbl_poli as p, tbl_diagnosa_penyakit as pkt, tbl_pasienumum as pas WHERE pas.no_rekamedis = '$no_rekmed' AND rt.no_rekamedis = '$no_rekmed' AND rt.kode_tindakan = t.kode_tindakan AND rt.id_poli = p.id_poli AND rt.kode_penyakit = pkt.kode_diagnosa";
      $sql_data_resep = "SELECT rt.nama_obat, rt.jenis_obat,rt.dosis_aturan_obat,rt.no_rawat,rt.tanggal, pkt.nama_pasien FROM tbl_resep_obat as rt, tbl_pasienumum as pkt WHERE rt.no_rawat = '$no_rawat' AND rt.no_rekamedis = pkt.no_rekamedis";
      ...
      $this->template->load('template', 'pendaftaranumum/detail', $data);
    ...
    function getNoRekamedis($namaPasien){
     ...
     function cetak(){
        $sql_data_resep = "SELECT * FROM tbl_pendaftaran";
...
function cetakbulan(){
    ...
    $sql_data_resep = "SELECT pd.*, pol.nama_poli, dok.nama_dokter, pas.nama_pasien FROM tbl_pendaftaran as pd, tbl_poli as pol, tbl_dokter as dok, tbl_pasienumum as pas where pd.kode_dokter=dok.kode_dokter AND pd.id_poli=pol.id_poli AND pd.no_rekamedis=pas.no_rekamedis AND MONTH(pd.tanggal_daftar)=MONTH(NOW()) order by pd.no_rawat asc";
  ...
  function cetakantrian($id){
        ...
        $sql_pasien = $this->Tbl_pendaftaranumum_model->get_by_id($id);
        $this->load->library('pdf');
        $pdf = new FPDF();
        $pdf->AddPage('P','A4');
    ...
    public function read() 
    {
        ...
        $row = $this->Tbl_pendaftaranumum_model->get_by_id($id);
        if ($row) {
            $data = array(
            ...
            $this->template->load('template','pendaftaranumum/tbl_pendaftaran_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('pendaftaranumum'));
        }
    }
   ...
   function autonorekamedis(){
        ...
        $datapasien = $this->db->get('tbl_pasienumum')->result();
   ...
   function rujukan_action(){
    ...
    redirect('pendaftaranumum/detail/'.$no_rawat);
   }

    public function create() 
    {
        $noreg = noRegistrasiotomatis2();
        $data = array(
        'button' => 'Tambah',
        'action' => site_url('pendaftaranumum/create_action'),
        ...
        $this->template->load('template','pendaftaranumum/tbl_pendaftaran_form', $data);
    }

    function cetakrekamedis(){

        $no_rawat = substr($this->uri->uri_string(3), 27);
      $sql_daftar = "SELECT pd.no_rekamedis, pd.no_rawat,pd.tanggal_daftar,ps.nama_pasien, td.nama_dokter FROM tbl_pendaftaran as pd, tbl_pasienumum as ps, tbl_dokter as td WHERE pd.no_rekamedis = ps.no_rekamedis and td.kode_dokter = pd.kode_dokter and pd.no_rawat = '$no_rawat'";
      $sql_penyakit = "SELECT pd.no_rekamedis, pd.no_rawat,dg.nama_penyakit FROM tbl_pendaftaran as pd, tbl_diagnosa_penyakit as dg WHERE pd.no_rekamedis and pd.no_rawat = '$no_rawat'";
      $sql_poli_input_rekmed = "SELECT pd.no_rawat,pd.id_poli, p.nama_poli FROM tbl_pendaftaran as pd, tbl_poli as p WHERE pd.no_rawat = '$no_rawat' AND pd.id_poli = p.id_poli";
    
      $no_rekmed = $this->db->query($sql_daftar)->row()->no_rekamedis; //untuk proses pengambilan data atribut no_rekamedis (yang diambil hanya data atribut itu saja)
      $sql_data_tindakan = "SELECT rt.no_rawat,rt.no_rekamedis,rt.hasil_periksa,rt.nama_obat,rt.tanggal,p.nama_poli, pkt.nama_penyakit, t.nama_tindakan FROM tbl_riwayat_tindakan as rt, tbl_tindakan as t, tbl_poli as p, tbl_diagnosa_penyakit as pkt, tbl_pasienumum as pas WHERE pas.no_rekamedis = '$no_rekmed' AND rt.no_rekamedis = '$no_rekmed' AND rt.kode_tindakan = t.kode_tindakan AND rt.id_poli = p.id_poli AND rt.kode_penyakit = pkt.kode_diagnosa";
      $sql_daftare = "SELECT pd.no_rekamedis, pd.no_rawat,ps.nama_pasien FROM tbl_pendaftaran as pd, tbl_pasienumum as ps WHERE pd.no_rekamedis = ps.no_rekamedis and pd.no_rawat = '$no_rawat'";
    ...
    $no_rawat = substr($this->uri->uri_string(3), 27);
      ...
      $sql_penyakit = "SELECT pd.no_rekamedis, pd.no_rawat,dg.nama_penyakit FROM tbl_pendaftaran as pd, tbl_diagnosa_penyakit as dg WHERE pd.no_rekamedis and pd.no_rawat = '$no_rawat'";
      $sql_poli_input_rekmed = "SELECT pd.no_rawat,pd.id_poli, p.nama_poli FROM tbl_pendaftaran as pd, tbl_poli as p WHERE pd.no_rawat = '$no_rawat' AND pd.id_poli = p.id_poli";
      $sql_data_resep = "SELECT rt.nama_obat, rt.jenis_obat,rt.dosis_aturan_obat,rt.jumlah_obat,rt.no_rawat,rt.tanggal, pkt.nama_pasien FROM tbl_resep_obat as rt, tbl_pasienumum as pkt WHERE rt.no_rawat = '$no_rawat' AND rt.no_rekamedis = pkt.no_rekamedis";
    ...
    public function create_action() 
    {
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->Tbl_pendaftaranumum_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('pendaftaranumum/update_action'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Tbl_pendaftaranumum_model->update($this->input->post('no_rawat', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tbl_pendaftaranumum_model->delete($id);
    ...
    foreach ($this->Tbl_pendaftaranumum_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'tbl_pendaftaran_data' => $this->Tbl_pendaftaranumum_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('pendaftaranumum/tbl_pendaftaran_doc',$data);
    }
